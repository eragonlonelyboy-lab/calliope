...
2. **Instrumented audit-redo loop before every checkpoint.** Build, render for real, inspect with instruments (not eyes alone), fix, re-inspect, repeat until ZERO findings, only then show the client. `references/quality/audit_redo_loop.md`.
